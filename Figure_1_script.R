...
blood<-read.table("Fig1_blood.txt",sep="\t",stringsAsFactors = F, 
                  header=T, check.names=F,as.is=T)
...
weekly_monitoring$Species<-"Mouse"
...
weeklyTab$Species<-sapply(weeklyTab$Factor,function(f) unique(weekly_monitoring$Species[weekly_monitoring$Factor==f]))
weeklyTab$Species<-factor(weeklyTab$Species,levels=levels(weekly_monitoring$Species))
...
pr <- matrix(c(0,16,log(0.01),log(45),0.99),byrow=T, nrow=1,ncol = 5)

save.image("./preAmelia.RData")

set.seed(12345)
GSIS.imp <- amelia(GSIS, m = 20, p2s=0, idvars = c("Date","Days","Delivery","InjDays","Weeks",#"CP",
                                                   "CP.End","CP.Start",
                                                   "BG.End","BG.Start",
                                                   "Factor","Species"),
...
GSIStab$Species<-sapply(GSIStab$Factor,function(f) unique(GSIS$Species[GSIS$Factor==f]))
